...
sudo dnf install -y container-tools sqlcl jdk-26-headless wget git
...
# inst3.sh invokes these scripts explicitly inside the database container.
# Enforce the mode after unzipping because archive permission metadata can vary.
chmod 755 /home/opc/ingestion/db-startup/*.sh
...
mkdir -p /home/opc/ingestion/runtime
...
echo "Host bootstrap complete. Podman Compose and database initialization have not started."
echo "Take the OCI boot-volume snapshot now, then run /home/opc/inst3.sh to start them."
