...
# Resolve one canonical DB password for all generated config files.
...
echo "DBUSER=SELECTAI_LAB" >> "$APP_DIR/.env"
echo "DBPASSWORD=${ORACLE_PWD_VALUE}" >> "$APP_DIR/.env"
echo "PASSWORD=${ORACLE_PWD_VALUE}" >> "$APP_DIR/.env"
...
echo "SELECTAI_PROVIDER=OLLAMA" >> "$APP_DIR/.env"
echo "SELECTAI_OLLAMA_MODEL=llama3.2" >> "$APP_DIR/.env"
echo "SELECTAI_OCI_MODEL=${SELECTAI_OCI_MODEL:-${OCI_GENAI_CHAT_MODEL:-}}" >> "$APP_DIR/.env"
...
printf 'SELECTAI_REQUIRE_OCI=%s\n' "${SELECTAI_REQUIRE_OCI:-true}" >> "${COMPOSE_ENV}"
printf 'USER_OCID=%s\n' "${USER_OCID}" >> "${COMPOSE_ENV}"
printf 'TENANCY_OCID=%s\n' "${TENANCY_OCID}" >> "${COMPOSE_ENV}"
printf 'PEM_KEY_FINGERPRINT=%s\n' "${PEM_KEY_FINGERPRINT}" >> "${COMPOSE_ENV}"
printf 'COMPARTMENT_OCID=%s\n' "${COMPARTMENT_OCID}" >> "${COMPOSE_ENV}"
printf 'AI_ENDPOINT_REGION=%s\n' "${AI_ENDPOINT_REGION}" >> "${COMPOSE_ENV}"
printf 'OCI_GENAI_CHAT_MODEL=%s\n' "${SELECTAI_OCI_MODEL:-${OCI_GENAI_CHAT_MODEL:-}}" >> "${COMPOSE_ENV}"
